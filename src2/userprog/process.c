...
process_execute (const char *file_name) 
{
  ...
  //added -> parsing file name
  strlcpy(file_all,file_name,strlen(file_name)+1);
  token = strtok_r(file_all, " ", &save_ptr);
...
start_process (void *file_name_)
{
  ...
  //added -> variable for parsing
  char *token, *save_ptr, *parse[(strlen(file_name)+1)/2];
  int count = 0;
  ...
  //added2 at lab2. It sema_up the exec sema in parent thread after the load above. 
  //(Might be problem)
...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  // thread_block();
  //child is running
  while(childThr->status==0){
    
  }
  sema_up(cur->parent->sema_wait);
  // thread_unblock(cur);
  return childThr->exit_status;
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  //added2
  t->runningFile= file = filesys_open (file_name);
